...
| (Sum.inr (Sum.inl g)) => (Sum.inl (Sum.inr g))
| (Sum.inr (Sum.inr a)) => (Sum.inr a)
...
def its_wet (rs : rain ⊕ sprinkler) (rainToWet : rain → wet) (sprinklerToWet : sprinkler → wet) : wet :=
match rs with
...
def sum_elim {α β γ : Type} (rs : α ⊕ β) (fα : α → γ) (fβ : β → γ) : γ :=
match rs with
  | Sum.inl a => fα a
  | Sum.inr b => fβ b
/-!
